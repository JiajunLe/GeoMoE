...
def test(config):
    ...
    save_file_best = os.path.join(config.model_file, "sun3d.pth")
    ...
    for index, test_data in enumerate(tqdm(test_loader)):
        ...
        logits_list, e_hat_list, _ = model(data)
        res_list.append(e_hat_list)
    ...
    all_time /= len(res_list)
    print("all_time:", all_time, "seconds.")

    precision_all = [precision.item() for precision in precision_all]
    recall_all = [recall.item() for recall in recall_all]
    f_scores_all = [f_scores.item() for f_scores in f_scores_all]
