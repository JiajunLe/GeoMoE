...
def valid_process(mode, model, cur_global_step, data_loader, config):
    ...
    network_infor_list = ["geo_losses", "cla_losses", "l2_losses", 'precisions', 'recalls', 'f_scores', 'loss_load_balance']
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            res_logits, res_e_hat, loss_load_balance = model(test_data)
            ...
            loss += loss_load_balance.item() * config.loss_balance
            info = [geo_loss, cla_loss, l2_loss, prec, rec, 2*prec*rec/(prec+rec+1e-15), loss_load_balance.item()]
    ...
    return [ret_val, np.mean(np.asarray(network_info['geo_losses'])), np.mean(np.asarray(network_info['cla_losses'])), \
        ...
        np.mean(np.asarray(network_info['loss_load_balance']))]
